...
x=1;
  ...
  for its = 1:max_its   % Note: no automatic convergence test
      ...
      idx = find(phi <= 1 & phi >= -1 & phi1>1);  %get the curve's narrow band
    idx1= find(phi1 <=1 & phi1 >= -1 & phi>1 );
    %-- find interior and exterior mean
    upts = find(phi<=0);                 % interior points
    vpts = find(phi>0);                  % exterior points
    upts1 = find(phi1<=0);                 % interior points
    vpts1 = find(phi1>0); 
    u = sum(I(upts))/(length(upts)+eps); % interior mean
    v = sum(I(vpts))/(length(vpts)+eps); % exterior mean
    u1 = sum(I(upts1))/(length(upts1)+eps); % interior mean
    v1 = sum(I(vpts1))/(length(vpts1)+eps);
    F = 5*(I(idx)-u).^2-(I(idx)-v).^2; 
    % force from image information
     F1=5*(I(idx1)-u1).^2-(I(idx1)-v1).^2;
    curvature = get_curvature(phi,idx);  % force from curvature penalty
    curvature1=get_curvature(phi1,idx1);
    dphidt = F./max(abs(F)) + alpha*curvature;  % gradient descent to minimize energy
    dphidt1 = F1./max(abs(F1)) + alpha.*curvature1;
    %-- maintain the CFL condition
    dt = 0.9/(max(dphidt)+eps);
    dt1 = 0.9/(max(dphidt1)+eps);
    ...
    if((display>0)&&(mod(its,20) == 0)) 
      showCurveAndPhi(I,phi,phi1,its);  
    end
  %end
  end
  %-- final output
  ...
  %-- make mask from SDF
  seg = phi<=0;
  seg1=phi1<=0;%-- Get mask from levelset
...
function showCurveAndPhi(I, phi, phi1,i)
  imshow(I,'initialmagnification',200,'displayrange',[0 255]); hold on;
  contour(phi, [0 0], 'g','LineWidth',1);
  contour(phi1, [0 0], 'r','LineWidth',1);
  %contour(phi2, [0 0], 'b','LineWidth',4);
  %contour(phi3, [0 0], 'y','LineWidth',4);
  %contour(phi4, [0 0], 'w','LineWidth',4);
  %contour(phi5, [0 0], 'm','LineWidth',4);
